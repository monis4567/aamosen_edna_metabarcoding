...
for(i in seq_along(fnFs)) {
 if (file.info(fnFs[i])$size == 0) {
  print(paste(fnFs[i], "empty.")) } else {
   ...
   commandX <- paste0("/groups/hologenomics/phq599/data/ES_metabarcode_example/Metabarcoding_pipeline/sickle-master/sickle se -l 100 -q 28 -x -t sanger -f ",fnFs[i]," -o ",filtFs[i])
   ...
   commandY <- paste0("/groups/hologenomics/phq599/data/ES_metabarcode_example/Metabarcoding_pipeline/sickle-master/sickle se -l 100 -q 28 -x -t sanger -f ",fnRs[i]," -o ",filtRs[i])
   print(commandX)
   system(commandX)
   print(commandY)
   system(commandY)
