...
#get count of unique elements for genus
ll01 <-length(unique(nochim_table_melt$blast_r_genus))
ll01 <-length(unique(nochim_table_melt$blast_r_fam))
ll01 <-length(unique(nochim_table_melt$blast_r_class))
ll02 <-length(unique(nochim_table_melt$blast_r_genus_species))
...
ncm_df2 <- nochim_table_melt[
  with(nochim_table_melt, order(blast_r_class,
                                blast_r_fam,
                                blast_r_genus_species)),
  ]
...
#assign color to coloumn in df matching the columns the colors were based on
ncm_df2$cg01 <- c(pal02)[ncm_df2$blast_r_class]
#count species
cntspcpgrp_df <- as.data.frame(table(ncm_df2$blast_r_genus_species))

#make fractions for each category based the table, divide with the total number per category
fr01 <- seq(1:((table(nochim_table_melt$blast_r_genus_species)[1])+1))
/(nochim_table_melt$blast_r_genus_species)[1]+1)
