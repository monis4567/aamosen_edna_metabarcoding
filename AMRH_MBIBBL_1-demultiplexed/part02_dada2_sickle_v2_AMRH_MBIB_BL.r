...
## Now that you have installed all packages in your remote path : "/groups/hologenomics/phq599/data/R_packages_for_Rv3_4"
...
## /groups/hologenomics/phq599/data/ES_metabarcode_example/1-demultiplexed-filtered/E16L
...
## navigate in to the directory : /groups/hologenomics/phq599/data/ES_metabarcode_example/1-demultiplexed-filtered/E16L
...
# define the lenght and quality for sickle
length_and_qual_set <- "-l 30 -q 60"
...
for(i in seq_along(fnFs)) {
 if (file.info(fnFs[i])$size == 0) {
  print(paste(fnFs[i], "empty.")) } else {
   ...
   commandX <- paste0("/groups/hologenomics/phq599/data/ES_metabarcode_example/Metabarcoding_pipeline/sickle-master/sickle se ",length_and_qual_set," -x -t sanger -f ",fnFs[i]," -o ",filtFs[i])
   ...
   commandY <- paste0("/groups/hologenomics/phq599/data/ES_metabarcode_example/Metabarcoding_pipeline/sickle-master/sickle se ",length_and_qual_set," -x -t sanger -f ",fnRs[i]," -o ",filtRs[i])
   print(commandX)
   system(commandX)
   print(commandY)
   system(commandY)
...
for(i in seq_along(fnFs)) {
 if (file.info(fnFs[i])$size == 0) {
  print(paste(fnFs[i], "empty.")) } else {
   print(paste("processing", fnFs[i]))
   fastqPairedFilter(c(fnFs[i], fnRs[i]), c(filtFs[i], filtRs[i]), minLen=100, maxN=0, maxEE=2, truncQ=2,
                     matchIDs=TRUE)
  }
}
