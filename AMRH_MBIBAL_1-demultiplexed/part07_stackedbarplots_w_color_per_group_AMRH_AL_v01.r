...
# my_wd <- "/home/hal9000/metabarcode_example/AMRH_MBIBAL_1-demultiplexed"
# #set working directory
# setwd(my_wd)
...
# first read in the renamed 'DADA2_nochim.table' 
#that has been renamed to : 'part03_DADA2_nochim.table.JMS_FL.txt'
inp_fn1 <- "part03_DADA2_nochim.table.AMRH_AL.txt"
#inp_fn2 <- "part06_my_classified_otus_JMSFL.01.unfilt_BLAST_results.txt"
inp_fn2 <- "part06_my_classified_otus_AMRHAL.01.unfilt_BLAST_results.txt"
#inp_fn3 <- "part06_my_classified_otus_JMSFL.02.filt_BLAST_results.txt"
inp_fn3 <- "part06_my_classified_otus_AMRHAL.02.filt_BLAST_results.txt"
# 
...
tbl_nochim01 <- read.table(file=inp_fn1,sep='\t',header=T)

#read in the result from the taxonomyB_R code
df_blast_unfilt_01 <- read.table(file=inp_fn2,sep='\t',header=T, stringsAsFactors = F)
df_blast_filt_01 <- read.table(file=inp_fn3,sep='\t',header=T, stringsAsFactors = F)
...
plot.nm3 <- paste("part07_outpt_",filn02,"02", sep="")
